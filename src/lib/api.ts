...
// ─── Dynamic API Base URL for Local, Vercel & Netlify ─────────────────────────

export const API_BASE = (() => {
  if (typeof window === 'undefined') return 'http://localhost:8000/api';
  // 1. Explicit env variable (Vercel/Netlify build-time env)
  if (import.meta.env.VITE_API_URL) return (import.meta.env.VITE_API_URL as string).replace(/\/+$/, '');
  // 2. Production Vercel / Netlify deployment (use relative /api route)
